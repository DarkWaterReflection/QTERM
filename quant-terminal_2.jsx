...
fontLink.href = 'https://fonts.googleapis.com/css2?family=IBM+Plex+Mono:wght@300;400;500;600;700&family=IBM+Plex+Sans:wght@300;400;500;600&display=swap';
...
globalStyle.textContent = `
  ...
  @keyframes ripple { 0%{box-shadow:0 0 0 0 rgba(239,68,68,0.5)} 70%{box-shadow:0 0 0 8px rgba(239,68,68,0)} 100%{box-shadow:0 0 0 0 rgba(239,68,68,0)} }
  @keyframes slideIn { from{opacity:0;transform:translateY(8px)} to{opacity:1;transform:translateY(0)} }
  @keyframes flash { 0%{background:rgba(245,158,11,0.12)} 100%{background:transparent} }
  body { margin:0; overflow:hidden; background:#040608; }
  * { box-sizing:border-box; }
  ...
  .spinner { width:18px;height:18px;border:2px solid #1a2530;border-top-color:#f59e0b;border-radius:50%;animation:spin 0.8s linear infinite; }
  .blink-dot span { animation:blink 1.4s infinite; }
  .blink-dot span:nth-child(2) { animation-delay:0.2s; }
  .blink-dot span:nth-child(3) { animation-delay:0.4s; }
  .live-dot { width:6px;height:6px;border-radius:50%;background:#10b981;box-shadow:0 0 6px #10b981;animation:pulse 2s infinite; }
  .ai-response-body { animation: slideIn 0.3s ease; }
  pre { margin:0; }
`;
...
function formatAIResponse(text) {
  ...
  return segments.map((seg, i) => {
    ...
    // text segment — parse markdown-ish
...
function HeatmapPanel() {
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'repeat(6,1fr)',gap:2,padding:8}}>
      {cells.map(c=>(
        <div key={c.sym} title={`${c.sym}: ${c.p}%`} style={{background:col(c.p),aspectRatio:1,display:'flex',flexDirection:'column',alignItems:'center',justifyContent:'center',borderRadius:2,cursor:'pointer',padding:3}}>
...
function GeoPanel() {
  ...
  return (
    <div>
      {GEO.map(r=>(
        <div key={r.name} style={{display:'flex',alignItems:'center',padding:'5px 10px',gap:8,borderBottom:`1px solid ${C.border}`,cursor:'pointer'}}>
  ...
  return (
    <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:1,background:C.border}}>
      {Object.entries(MACRO).map(([k,v])=>(
        <div key={k} style={{background:C.bg2,padding:'7px 10px',cursor:'pointer'}}>
  ...
  const renderContent = () => {
    ...
    if (view === 'overview') return (
      <div style={{ flex:1, display:'grid', gridTemplateColumns:'1fr 1fr 1fr', gridTemplateRows:'1fr 1fr', gap:1, background:C.border, overflow:'hidden' }}>
        ...
        <PanelShell title="SECTOR HEATMAP" actions={[{label:'1D',active:true},{label:'1W'},{label:'1M'}]}><HeatmapPanel /></PanelShell>
        ...
        <PanelShell title="LIVE NEWS & ALPHA SIGNALS" actions={[{label:'ALL',active:true},{label:'GEO'},{label:'MACRO'}]}><NewsPanel /></PanelShell>
      </div>
    );
